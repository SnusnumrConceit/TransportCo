    ...
    btnSender.click(function () {  
        var id = window.location.search.split('='),
            ...
            if (ValidateNumber(number) && ValidatePhoto(photo) && ValidateRoute(route)) {
                var bus = new Bus(id, number, route, statement);
                ...
                function Bus (id, number, route, statement) {
                    this.id =id[1],
                    this.number = number,
                    this.route = route,
    ...
    function ValidateNumber(number) {
        try {
            ...
            if (error.message === 'Length Number Error') {
                alert('Номер автобуса долен быть длиной 4 символа!');
            }
            
            if (error.message === 'Uncorrect Number Error') {
                alert('Номер автобуса должен состоять из цифр!');
            }
        }
    }
